...
def load_dataset_samples(path, text_col=None, max_samples=1000):
    samples = []
    ...
    samples = [s for s in samples if isinstance(s, str) and len(s) > 10 and len(s) < 2000]
...
malicious_datasets = [
    ...
    # 新增数据集
    {"name": "JailbreakHub+", "path": "datasets/jailbreakhub/all_jailbreak_prompts.csv", "text_col": "prompt"},
]

all_malicious_samples = []
for ds in malicious_datasets:
    if os.path.exists(ds["path"]):
        samples = load_dataset_samples(ds["path"], ds.get("text_col"), max_samples=1000)
        all_malicious_samples.extend(samples)
        print(f"  {ds['name']}: {len(samples)}条")

# 去重
all_malicious_samples = list(set(all_malicious_samples))
print(f"  恶意样本总计(去重后): {len(all_malicious_samples)}条")
...
all_normal_samples.extend(SECURITY_EDUCATION_SAMPLES * 20)
print(f"  安全教育样本: {len(SECURITY_EDUCATION_SAMPLES) * 20}条")
...
def v4_loss(projection, mal_emb, norm_emb, separation_weight=2.0, intra_weight=1.0):
    ...
    separation_loss = (1 + centroid_sim)
...
np.save(f"{save_dir}/learned_projection_weights_19d_v4_enhanced.npy", weights)
...
np.save(f"{save_dir}/centroid_19d_malicious_enhanced.npy", mal_centroid)
np.save(f"{save_dir}/centroid_19d_normal_enhanced.npy", norm_centroid)

# 计算阈值
with torch.no_grad():
    norm_proj_np = projection(norm_tensor).numpy()
    norm_sims = [np.dot(e, mal_centroid) for e in norm_proj_np]
    threshold_95 = np.percentile(norm_sims, 95)
    threshold_99 = np.percentile(norm_sims, 99)
...
print(f"  恶意质心已保存")
print(f"  正常质心已保存")
print(f"  检测阈值(95%): {threshold_95:.4f}")
print(f"  检测阈值(99%): {threshold_99:.4f}")
...
with open(f"{save_dir}/v4_enhanced_config.json", "w") as f:
    json.dump(config, f, indent=2)

print("\n" + "=" * 80)
print("V4增强版训练完成!")
print(f"恶意样本: {len(mal_tensor)}条")
print(f"正常样本: {len(norm_tensor)}条")
